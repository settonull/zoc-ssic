...
from gan_based_classifier import *
'''
...
def main():
	...
	if not if_gpu:
		...
		#clsfer = torch.load(args['save_path']+'classifier_epoch_250.pth', map_location = 'cpu');	
	else:
		netD.load_state_dict(torch.load(args['encoder_path']));
		#clsfer = torch.load(args['save_path']+'classifier_epoch_250.pth');	
	...
	if not if_gpu:
		clsfer.load_state_dict(torch.load(args['save_path']+'classifier_epoch_250.pth', map_location = 'cpu'));
	else:
		clsfer.load_state_dict(torch.load(args['save_path']+'classifier_epoch_250.pth'));
